...
partial_list* create_partial_list(arena* cur_arena){
  partial_list* addr_list;
  addr_list->position = borrow_mem(cur_arena, sizeof(size_t)*20, 1);
  addr_list->size = borrow_mem(cur_arena, sizeof(size_t)*20, 1);
  addr_list->next_position = borrow_mem(cur_arena, sizeof(size_t)*20, 1);
  addr_list->counter = 0;
  return addr_list;
}
...
void add_partial_list(arena* cur_arena, partial_list* cur_list, void* ptr, size_t length){
  ...
  cur_list->size[cur_list->counter] = length;
  if ()
}
